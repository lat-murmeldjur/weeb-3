...
self.onconnect = async function (event) {
  ...
  port.onmessage = async function (e) {
    ...
    if (e.data.type0 == "file") {
      var workerResultPromise0 = sekirei.post_upload(e.data.file0, e.data.encryption0, e.data.index0, e.data.feed0, e.data.topic0);
      ...
      var workerResultPromise2 = sekirei.reset_stamp();
      port.postMessage(await workerResultPromise2);
    } else {
